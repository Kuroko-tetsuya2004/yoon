...
export default function CreatePondereux({ auth, reperes, produit, flash }) {
    ...
    const submit = (e) => {
        ...
        if (data.mode_paiement !== 'especes') {
            ...
            post(route('commandes.pondereux.store', { ...data, frais_livraison: fraisLivraison }));
        }
    };
    ...
    return (
        <AuthenticatedLayout header={<h2 className="font-semibold text-xl text-gray-800 leading-tight">Commander un produit pondéreux</h2>}>
            <Head title="Commande Pondéreux" />
                    ...
                    >
                        ...
                        <form onSubmit={submit} className="space-y-6">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    {reperes.map(repere => (
                                        <label key={repere.id} className="relative block cursor-pointer">
                                            ...
                                            />
                                            <div className="p-4 rounded-lg border-2 border-gray-200 hover:bg-gray-50 peer-checked:border-yellow-500 peer-checked:bg-yellow-50 transition">
                                                <div className="flex justify-between items-center mb-1">
                                                    <span className="font-bold text-gray-900">{repere.nom}</span>
                                                    {repere.is_default && <span className="text-xs bg-yellow-100 text-yellow-800 px-2 py-0.5 rounded-full font-medium">Par défaut</span>}
                                                </div>
                            ...
                            <div className="mb-6">
                                ...
                                <div className="bg-gray-50 border border-gray-200 rounded-lg p-4 flex items-center">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-sm font-medium text-yellow-600 mt-1">Fournisseur : {produit.partenaire?.name}</p>
                            ...
                            <div className="mb-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    <label htmlFor="quantite" className="block font-medium text-sm text-gray-700 mb-2">Quantité (Camions/Sacs)</label>
                                    <input 
                                        type="number"
                                        id="quantite" 
                                        min="1"
                                ...
                                <div>
                                    <label htmlFor="poids_estime" className="block font-medium text-sm text-gray-700 mb-2">Poids estimé en kg (Optionnel)</label>
                                    <input 
                                        type="number"
                                        id="poids_estime"
                                        step="0.1"
                                        value={data.poids_estime}
                                        onChange={e => setData('poids_estime', e.target.value)}
                                        className="border-gray-300 focus:border-yellow-500 focus:ring-yellow-500 rounded-md shadow-sm w-full"
                                        placeholder="Ex: 500"
                                    />
                                    {errors.poids_estime && <p className="mt-2 text-sm text-red-600">{errors.poids_estime}</p>}
                                </div>
                            </div>
                            ...
                            <div className="mb-8 grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div>
                                    ...
                                    {requiresOnlinePayment && (
                                        <p className="mt-2 text-xs text-yellow-700 font-medium bg-yellow-50 p-2 rounded">
                            ...
                            <div className="flex items-center justify-between pt-4 border-t border-gray-200">
                                ...
                                <button 
                                    type="submit" 
                                    disabled={processing}
                                    className="bg-yellow-500 hover:bg-yellow-600 text-white font-bold py-3 px-8 rounded-lg shadow-lg transition transform hover:-translate-y-0.5 text-lg disabled:opacity-50"
                                >
